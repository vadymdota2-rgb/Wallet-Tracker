 ...
 *
 * Картинка — анимация, и она же единственное, что здесь движется само.
 * Поэтому при `prefers-reduced-motion` подписи перестают мелькать и просто
 * стоят на своих местах: мигающий текст читать тяжело и не всем безопасно.
 */
...
const SLOTS = 5;
/** Сколько одна подпись держится на экране. */
const STEP = 1400;
  ...
  return (
    <div className="brain" aria-hidden="true">
      ...
      {Array.from({ length: SLOTS }, (_, i) => {
        /* У каждого места своя очередь имён, и очереди сдвинуты: иначе все
           пять подписей менялись бы разом и читались бы как одна вспышка. */
        const n = names.length;
        const at = ((tick + i * Math.ceil(n / SLOTS)) % n + n) % n;
        const raw = names[at] ?? "";
        return (
          <span
            key={`${i}-${still ? "still" : tick}`}
