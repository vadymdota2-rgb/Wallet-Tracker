 ...
 *
 * Набор тот же, что на биржевых графиках, кроме пятнадцати минут: бот пишет
 * цену раз в час, и пятнадцатиминутной свечи из этого не получить никак.
 * Показать её пустой значило бы обещать данные, которых нет.
 *
 * Часовая здесь есть: у такой свечи одна точка, тела и теней нет — рисуется
 * ниточка. Это честно: столько и известно.
 */
export type SpotTf = "1h" | "4h" | "1d" | "1w" | "1M";
export const SPOT_TFS: SpotTf[] = ["1h", "4h", "1d", "1w", "1M"];
const SPOT_STEP: Record<SpotTf, number> = {
  "1h": 3600,
  "4h": 4 * 3600,
  "1d": 86400,
  "1w": 7 * 86400,
